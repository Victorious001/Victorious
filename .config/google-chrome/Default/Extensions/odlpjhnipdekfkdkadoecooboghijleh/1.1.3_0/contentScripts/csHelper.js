...
chrome.runtime.sendMessage("isInstaHelperEnabled", (msg) => {
  if (msg && msg.isInstaHelperEnabled) {
    function createScriptTag() {
      ...
      scriptTag.innerText += `Object.defineProperty(window.screen.orientation, 'type', { get: function(){ return 'portrait-primary'; } });`;
    ...
    function getImageFromTagged() {
      let array = [];
      array.push(...$(document).find("article").find("img").toArray());
      ...
      _removeFromArray(
        array,
        $(document).find("article").find("img.ignore").toArray()
      );

      _removeFromArray(
        array,
        $(document).find("article").find("header").find("img").toArray()
      );
      ...
      _removeFromArray(
        array,
        $(document).find("article").find('img[src*="static/images"]').toArray()
      );
    ...
    $(document).on("click", ".desc-photo", function (e) {
      ...
      chrome.runtime.sendMessage("download", (response) => {
        if (response.download === true) {
          let elements = $(this).parent().find("div");
          let number = 0;
          let photoWidth = $(this).parent().closest("body").width();
