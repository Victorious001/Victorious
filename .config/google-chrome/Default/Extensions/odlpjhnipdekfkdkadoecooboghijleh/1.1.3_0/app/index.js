...
function getAndSetInstaCookies() {
  const instaFirstCookie = localStorage.getItem("insta_first")
    ? localStorage.getItem("insta_first")
    : Date.now();
  const instaCountCookie = localStorage.getItem("insta_count")
    ? parseInt(localStorage.getItem("insta_count"))
    : 0;
...
async function appStart() {
  ...
  chromeBgPage.loadInstagram().then((res) => {
    ...
    document
      ...
      .addEventListener("click", goStepBack);
    document.getElementById("rate-btn").addEventListener("click", openRarePage);
    document
      .getElementById("accept")
      .addEventListener("click", acceptTermsOfUse);
    document
      .getElementById("decline")
      .addEventListener("click", declineTermsOfUse);
    document
      .getElementById("goBackTermsOfUse")
      .addEventListener("click", goBackTermsOfUse);
...
const bannerDismissalLocalStorageKey = "banner-dismissed";
async function showBanner() {
  ...
  const isoConfiguration = configuration[iso];
  if (!isoConfiguration) {
    return;
  }

  if (now.valueOf() > new Date(isoConfiguration.expiration).valueOf()) {
    return;
  }

  const shouldShowBanner = (() => {
    const dismissedRaw = localStorage.getItem(bannerDismissalLocalStorageKey);
    if (dismissedRaw) {
      const dismissed = JSON.parse(dismissedRaw);

      if (dismissed.version === isoConfiguration.banner.img_src) {
        const hoursSinceDismiss = differenceInHours(
          new Date(dismissed.at),
          now
        );
        switch (dismissed.count) {
          case 1: {
  ...
  $(".banner .dismiss-btn").show();
  $(".banner .dismiss-btn").click(() => {
    const existingDataRaw = localStorage.getItem(
      bannerDismissalLocalStorageKey
    );
    ...
    localStorage.setItem(
      ...
      JSON.stringify({
        ...
        version: isoConfiguration.banner.img_src,
      })
    );
  ...
  $(".banner").append(`
    <a class="banner-link" href="${isoConfiguration.banner.target}" target="_blank" rel="noopener noreferrer">
        <img src="${isoConfiguration.banner.img_src}" alt="${isoConfiguration.banner.text}" title="${isoConfiguration.banner.text}">
    </a>
  `);
}
...
function goRarePage() {
  ...
  localStorage.setItem("rateUs", JSON.stringify(rateUs));
  window.open(
    "https://chrome.google.com/webstore/detail/ig-desktop/odlpjhnipdekfkdkadoecooboghijleh/reviews",
    "_blank"
  );
}
...
function notificationInfoClick() {
  ...
  $(".show-second-note").click(function () {
    ...
    if (!shownNotif2) $("#snackbar-agree-2").show();
    else $("#snackbar-agree-2").hide();
